...
function TestSuite(significant_level){
  ...
  testNames.forEach(name => {
    this[name] = testCaseFactory(name, pvalues[name], significant_level)
  });
}

TestSuite.testNames = testNames;

TestSuite.prototype.runAll = function(bits){
  var ret = {};
  testNames.forEach(name => {
    ret[name] = this[name](bits);
  });
  return ret;
};
